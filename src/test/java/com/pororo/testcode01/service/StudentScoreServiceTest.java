...
class StudentScoreServiceMockTest {
    ...
    public void beforeEach(){
        ...
        studentScoreService = new StudentScoreService(studentScoreRepository,
                studentPassRepository,studentFailRepository);
    }
    ...
    public void saveScoreMockTest() {
        ...
        //when
        studentScoreService.saveScore(givenStudentName,
                givenExam, givenKorScore, givenEnglishScore, givenMathScore);
    ...
    void getPassStudentsList() {
        ...
        Mockito.when(studentPassRepository.findAll()).thenReturn(
                List.of(studentPass, studentPass2, studentPass3)
        );

        List<ExamPassStudentResponse> list = Stream.of(studentPass, studentPass2)
                .map((pass) -> new ExamPassStudentResponse(pass.getStudentName(), pass.getAvgScore())).toList();
        List<ExamPassStudentResponse> passStudentsList = studentScoreService.getPassStudentsList(givenTestExam);
    ...
    void getFailStudentsList() {
        ...
        Mockito.when(studentFailRepository.findAll()).thenReturn(
                List.of(studentFail, studentFail2, studentFail3)
        );

        List<ExamFailStudentResponse> list = Stream.of(studentFail, studentFail2)
                .map((fail) -> new ExamFailStudentResponse(fail.getStudentName(), fail.getAvgScore())).toList();
        List<ExamFailStudentResponse> passStudentsList = studentScoreService.getFailStudentsList(givenTestExam);
    ...
    public void saveScoreMockArgTest() {
        ...
        //when
        studentScoreService.saveScore(expactedStudentScore.getStudentName(),
        ...
        Assertions.assertEquals(expactedStudentScore.getStudentName(), capturedStudentScore.getStudentName());
        ...
        Assertions.assertEquals(expactedStudentScore.getEnglishScore(), capturedStudentScore.getEnglishScore());
        Assertions.assertEquals(expactedStudentScore.getMathScore(), capturedStudentScore.getMathScore());
